...
RUN su - builder -c "bash -lc 'eval \"$(/home/linuxbrew/.linuxbrew/bin/brew shellenv)\" && brew install pyenv'"
...
RUN export PYENV_ROOT=/home/builder/.pyenv && \
    ...
    eval "$(pyenv init -)" && \
    PYTHON_CONFIGURE_OPTS="${PYTHON_CONFIGURE_OPTS}" CFLAGS="${CFLAGS}" LDFLAGS="${LDFLAGS}" pyenv install -v 3.14.0 && \
    pyenv global 3.14.0
